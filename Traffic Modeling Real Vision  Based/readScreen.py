...
class State_Lengths:
	...
	def __init__(self):
		...
		self.mon = {'top': 140, 'left': 100, 'width': 884, 'height': 900}
	...
	def warped_simulation(self,rect, frame):
	...
	def subtractImage(self,image,SIDE):
	...
	def get_tails(self):
		...
		while 1:
		    ...
		    warp_upperlane = self.warped_simulation(upper_lane, image)
		    cv2.imshow("upper", warp_upperlane)

		    warp_lowerlane = self.warped_simulation(lower_lane, rotated180)
		    cv2.imshow("lower", warp_lowerlane)

		    warp_rightlane = self.warped_simulation(right_lane, rotated90)
		    cv2.imshow("right", warp_rightlane)

		    warp_leftlane = self.warped_simulation(left_lane, rotated270)
		    ...
		    mask_upper = self.subtractImage(warp_upperlane, "upper")
		    mask_lower = self.subtractImage(warp_lowerlane, "lower")
		    mask_right = self.subtractImage(warp_rightlane, "right")
		    mask_left = self.subtractImage(warp_leftlane, "left")


		    tail_length_upper = self.tail_length(mask_upper)
		    tail_length_lower = self.tail_length(mask_lower)
		    tail_length_right = self.tail_length(mask_right)
		    tail_length_left = self.tail_length(mask_left)
		    ...
		    #return(tail_length_upper,tail_length_lower,tail_length_left,tail_length_right)
